...
export const rawAltarMistConfig = {
  // 1. Прозрачность и плотность (мягкое аддитивное свечение):
  opacity: 0.38,          // Общая прозрачность тумана (0.1 - тончайший флер, 0.38 - оптимум, 0.7 - густой туман)
  puffDensity: 0.85,      // Плотность объемных клубов тумана

  // 2. Размеры зон маскировки:
  groundRadius: 3.10,     // Радиус стелющейся подушки у основания алтаря (в метрах, полностью накрывает стыки)
  puffRadius: 1.45,       // Радиус объемных клубов тумана
  puffHeight: 0.60,       // Высота подъема клубов тумана над пустышками

  // 3. Скорость движения:
  flowSpeed: 0.30,        // Скорость мягкого перекатывания и клубящегося движения

  // 4. Позиционирование и масштабирование:
  scale: [1.0, 1.0, 1.0], // Общий масштаб эффекта [X, Y, Z] или число
  offsetFog1: [0, -0.02, 0],     // Смещение относительно пустышки fog1 [X, Y, Z]
  offsetFire001: [0, -0.02, 0],  // Смещение относительно пустышки fire.001 [X, Y, Z]

  // 5. Атмосферная палитра тумана (#7b8a9c - оригинальный цвет тумана сцены):
  groundColor: new THREE.Color(0x7b8a9c), // Мягкий серебристо-дымчатый цвет тумана сцены (#7b8a9c)
  puffColor: new THREE.Color(0x7b8a9c),   // Цвет клубов тумана (#7b8a9c)
  rimColor: new THREE.Color(0x9cb5ce),    // Нежный серебристо-голубой ореол на просвет

  // 6. Управление жестким мешем FOG004:
  hideFOG004: true        // Скрыть плоский статичный FOG004 из Blender (устраняет все прямые ребра)
};

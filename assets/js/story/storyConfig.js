...
export const rawAltarMistConfig = {
  // 1. Прозрачность и плотность (воздушная, нежная полупрозрачность без эффекта воды):
  opacity: 0.22,          // Общая прозрачность облачков (0.12 - тонкий флер, 0.22 - мягкая тучка, 0.4 - плотный дым)
  puffDensity: 0.85,      // Плотность клубов тумана

  // 2. Размеры объемных облачков ("туман-тучка") и их распределение:
  ...
  groundRadius: 2.50,     // Для обратной совместимости консоли

  // 3. Скорость органического покачивания и клубящегося движения:
  flowSpeed: 0.25,        // Скорость плавного дыхания тумана
  ...
  hideFOG004: true        // Скрыть плоский статичный FOG004 из Blender (устраняет все прямые ребра и полосы)
};

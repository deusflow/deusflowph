...
/* ====================================================================
 * 4. ALTAR MIST & DENSE FOG CARPET ("ПЛОТНЫЙ КОВЕР ТУМАНА НА АЛТАРЕ") CONFIGURATION
 * НАСТРОЙКИ ПЛОТНОГО КОВРА ТУМАНА НА ПЛИТЕ И ПОЛУ АЛТАРЯ:
 * Меняйте параметры прямо здесь в коде, либо в консоли браузера:
 * ALTAR_MIST_CONFIG.<параметр> = ...
 * ==================================================================== */
export const rawAltarMistConfig = {
  // 1. Прозрачность и плотность ковра тумана на алтаре:
  // 0.65 или 65% — естественный, мягкий и плотный ковер тумана
  // 1.0 или 100% — ультра-плотный белоснежный ковер
  // Значения > 1.0 (например 10.45) автоматически переводятся в 100% плотность с усилением густоты!
  opacity: 0.65,          // Прозрачность ковра тумана на алтаре (0.0..1.0, либо 0..100%)
  carpetDensity: 1.05,    // Насыщенность и густота ковра (1.05 - живой струящийся слой)
  flowSpeed: 0.38,        // Скорость мягкого перекатывания дымки по плите и камням
  yOffset: 0.02,          // Микро-подъем (2 см) над плитами пола для устранения мерцания (z-fighting)

  // 2. Геометрия ковра и пара:
  carpetSpread: 1.5,      // Мягкое растекание по плите алтаря
  steamHeight: 0.40,      // Мягкий невысокий подъем пара над чашей (фокус на плотном ковре)
  steamRadius: 0.40,      // Ширина дымки над чашей

  // 3. Светлая сияющая палитра (без черноты, плотный перламутровый ковер):
  color: new THREE.Color(0xd5e8f7),       // Светлый шелковистый туман (#d5e8f7)
  coreColor: new THREE.Color(0xa5e0f7),   // Нежный бирюзовый отблеск у чаши
  rimColor: new THREE.Color(0xf5faff),    // Перламутровые тающие гребни
  puffColor: new THREE.Color(0xd5e8f7),   // Для обратной совместимости
  groundColor: new THREE.Color(0xd5e8f7), // Для обратной совместимости

  // 4. Напольная плоскость FOG.004 на алтаре:
  hideFOG004: false,      // Напольный ковер FOG.004 (false = активен с плотным бесшовным шейдером)

  // 4b. Объемный кластер тумана в каньоне (замена FOG.007 без врезания в скалы):
  canyonMistDensity: 1.10, // Насыщенность тумана в каньоне
  canyonMistSpread: 1.0,   // Масштаб ширины клубов в каньоне
  canyonMistYOffset: 0.0,  // Коррекция высоты тумана в каньоне
